...
# comm_preps_1, comm_1, comp_1 = process_pair(1, size)
comm_preps_2, comm_2, comp_2, gpu_2 = process_pair(2, size)
comm_preps_3, comm_3, comp_3, gpu_3 = process_pair(3, size)

width = 0.25

plt.bar(mpi_procs, comm_0, width, label="Naive (comm)")
plt.bar(mpi_procs, comp_0, width, bottom=comm_0, label="Naive (comp)")

plt.bar(mpi_procs + width, comm_2, width, label="DGEMM (comm)")
plt.bar(mpi_procs + width, comp_2, width, bottom=comm_2, label="DGEMM (comp)")

plt.bar(mpi_procs + 2 * width, comm_3, width, label="cuBLAS DGEMM (comm)")
plt.bar(
    mpi_procs + 2 * width,
    gpu_3,
    width,
    bottom=comm_3,
    label="cuBLAS DGEMM (GPU transfer)",
)
plt.bar(
    mpi_procs + 2 * width,
    comp_3,
